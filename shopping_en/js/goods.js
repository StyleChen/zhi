...
$(function () {
    ...
    var goodsData = {series:series,key:key};
        //默认排序
        ...
        shopAjax("get","goods/all",goodsData,function (res) {
            if(res.code == 0){
                ...
                for(var i=0;i<arr.length;i++){
                    var sums = arr[i].usFakePrice.split('|');
                    var olds = arr[i].usNewPrice.split('|');
                    html += '<li><input type="hidden" class="goodsId" value="'+arr[i].id+'"> <img src="'+arr[i].showImage+'" alt=""> <h2>'+arr[i].name+'</h2> <div class="goodsMoney"> <strong>$'+sums[0]+'</strong> <em>$'+olds[0]+'</em> <span>sales'+arr[i].sellCount+'pieces</span> </div> </li>'
                }
