...
$(function () {
    ...
    shopAjax("post","cart/load",{userId:userId},function (res) {
        ...
        if(res.code == 0){
            ...
            for(var arr=0;arr<data.length;arr++){
                content += '<div class="cartItem"><input type="hidden" class="hidden" value="'+data[arr].goodsId+'"> ' +
                    ...
                    '<p class="productSize">size：<span>'+data[arr].size+'</span></p> ' +
                    '<p class="productColor">color：<span>'+data[arr].color+'</span></p> ' +
                    '</div> ' +
                    '<div class="left productPrice">$<span class="price">'+parseFloat(data[arr].usNewPrice)+'</span></div> ' +
                    '<div class="left productNumber"> ' +
                    '<span class="minus">-</span> ' +
                    '<input type="number" name="productNumber" value="'+data[arr].count+'"> ' +
                    '<span class="plus">+</span> ' +
                    '</div> ' +
                    '<div class="left productSubtotal">$<span class="productSubtotalMoney">'+(parseFloat(data[arr].usNewPrice) * data[arr].count).toFixed(2)+'</span></div> ' +
                    '<div class="left productOperation">delete</div> ' +
                    '</div>';
            }
        ...
        if (res.code == 1){
            $(".cartDetails").html("<p style='margin-top: 30px;margin-left: 30px;'>The shopping cart is empty. Pick up your favorite items quickly</p>");
        }
    });
